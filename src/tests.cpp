...
#define FOREACH_EXTRA_FUNC(X) X("XCgsqzn000004d", void, avs_fs_umount_by_desc, uint32_t desc)
...
class Environment : public ::testing::Environment {
  public:
    ...
    void TearDown() override { avs_standalone::shutdown(); }
};
...
class DevModeOnOff : public testing::TestWithParam<bool> {
    void SetUp() override { config.developer_mode = GetParam(); }
    void TearDown() override { config.developer_mode = false; }
};
...
class TestHookFile final : public HookFile {
  ...
  public:
    ...
    uint32_t call_real() override { throw "Unimplemented"; }

    std::optional<std::vector<uint8_t>> load_to_vec() override { throw "Unimplemented"; }
};
...
TEST_P(DevModeOnOff, CaseInsensitiveFiles) {
    EXPECT_THAT(find_first_modfile("OhNo/oWo"),
        Optional(config.get_mod_folder() / "Case_Sensitive/OhNO/oWo"));
    EXPECT_THAT(find_first_modfile("ohno/owo"),
        Optional(config.get_mod_folder() / "Case_Sensitive/OhNO/oWo"));
}
...
TEST(ImageFs, MD5DemanglingWorks) {
    ...
    EXPECT_EQ(
        lookup_tex("tex", "inner"), config.get_mod_folder() + "/md5_lookup/test_ifs/tex/inner.png");
    EXPECT_EQ(
        lookup_tex("tex", "outer"), config.get_mod_folder() + "/md5_lookup/test_ifs/outer.png");
    EXPECT_EQ(lookup_afp("afp", "confirm_all"),
        config.get_mod_folder() + "/md5_lookup/test_ifs/afp/confirm_all");
    EXPECT_EQ(lookup_afp("afp/bsi", "confirm_all"),
        config.get_mod_folder() + "/md5_lookup/test_ifs/afp/bsi/confirm_all");
    EXPECT_EQ(lookup_afp("geo", "confirm_all_shape5"),
        config.get_mod_folder() + "/md5_lookup/test_ifs/geo/confirm_all_shape5");
    EXPECT_EQ(lookup_afp("geo", "confirm_all_shape11"),
        config.get_mod_folder() + "/md5_lookup/test_ifs/geo/confirm_all_shape11");
...
TEST(LongPath, AvsFsOpenOver128Chars) {
    std::string subpath = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
                          "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.bin";
...
TEST(RamFs, DemanglingWorksNabla) {
    ...
    ramfs_demangler_on_fs_mount(
        "/mnt/bm2d/rfs88r89/logo.ifs", "/mnt/bm2d/rmp_89_2714881/image.bin", "link", "");
    ramfs_demangler_on_fs_mount(
        "/mnt/bm2d/ngp88/logo.ifs", "/mnt/bm2d/rfs88r89/logo.ifs", "imagefs", std::nullopt);
...
TEST(ArcArchive, IfsWithExtraOverrides) {
    ...
    std::vector<uint8_t> expected_override = {'o', 'v', 'e', 'r', 'r', 'i', 'd', 'd', 'e', 'n', '_',
        'o', 't', 'h', 'e', 'r', '_', 'd', 'a', 't', 'a'};
...
TEST(Regression, BeatStreamAfpXml) {
    ...
    int mnt = hook_avs_fs_mount(
        "/afp22/data2/graphic/psd_result.ifs", "/data2/graphic/psd_result.ifs", "imagefs", nullptr);
    ASSERT_GT(mnt, 0);

    auto f = hook_avs_fs_open(
        "/afp22/data2/graphic/psd_result.ifs/afp/afplist.xml", avs_open_mode_read(), 420);
    ASSERT_GT(f, 0);
    rapidxml::xml_document<> xml_doc;
    bool res = rapidxml_from_avs_file(f, xml_doc, xml_doc);
    ASSERT_TRUE(res);
