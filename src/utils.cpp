...
CacheHasher::CacheHasher(std::filesystem::path _hash_file)
    : hash_file(std::move(_hash_file)) {
...
void CacheHasher::finish() { digest.getHash(new_hash.data()); }

bool CacheHasher::matches() { return new_hash == existing_hash; }

void CacheHasher::commit() { write_bytes(hash_file, new_hash); }
