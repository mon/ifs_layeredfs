...
#define TEST_HOOK_AND_APPLY(func)                                                                  \
    if (MH_CreateHookApi(dll_name, exports.func, (LPVOID)hook_##func, (LPVOID*)&func) != MH_OK ||  \
        func == NULL)                                                                              \
    continue
#define LOAD_FUNC(func)                                                                            \
    if ((func = (decltype(func))GetProcAddress(mod_handle, exports.func)) == NULL)                 \
    continue
#define CHECK_UNIQUE(func)                                                                         \
    if (exports.func != NULL && GetProcAddress(mod_handle, exports.func) == NULL)                  \
    continue

#define AVS_FUNC_LOAD(ret_type, name, ...) LOAD_FUNC(name);
#define AVS_FUNC_LOAD_OPTIONAL(ret_type, name, ...)                                                \
    name = (decltype(name))GetProcAddress(mod_handle, exports.name);
...
#define VERBOSE_EXPORT_CHECK(ret_type, name, ...)                                                  \
    if (exports.name == NULL)                                                                      \
        log_warning("MISSING EXPORT {}: {}", i, #name);
...
bool rapidxml_from_avs_filepath(const char* path, rapidxml::xml_document<>& doc,
...
bool rapidxml_from_avs_file(
    AVS_FILE f, rapidxml::xml_document<>& doc, rapidxml::xml_document<>& doc_to_allocate_with) {
...
std::optional<std::vector<uint8_t>> lz_decompress(
    std::span<const uint8_t> input, size_t expected_length) {
