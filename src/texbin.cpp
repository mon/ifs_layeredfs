...
class Magic {
  public:
    ...
    bool operator==(std::string_view s) { return std::string_view(magic) == s; }
...
static std::vector<uint8_t> argb8888_to_texture_data(
    const unsigned char* image, unsigned width, unsigned height, bool little_endian = true) {
...
bool Texbin::add_or_replace_image(const char* image_name, const char* png_path) {
    ...
    if (existing_rect != rects.end()) {
        if (width != existing_rect->second.w || height != existing_rect->second.h) {
            log_info(
                "Replacement rect image {} has dimensions {}x{} but original is {}x{}, ignoring",
        ...
        if (width != w || height != h) {
            log_info(
                "Replacement image {} has dimensions {}x{} but original is {}x{}, repacking anyway",
...
void Texbin::debug() {
    ...
    for (auto& [name, image] : images) {
        ...
        VLOG("file: %s len %d fmt %s dims(%d,%d)", name.c_str(), image.tex.size(),
            image.tex_type_str(false).c_str(), w, h);
        total += (uint32_t)image.tex.size();
    }

    for ([[maybe_unused]] auto& [name, rect] : rects) {
        VLOG("rect: %s parent %s dims (%d,%d,%d,%d)", name.c_str(), rect.parent_name.c_str(),
...
std::optional<Texbin> Texbin::from_stream(std::istream& f) {
    ...
    if (hdr.archive_size != file_len) {
        log_warning("bad archive size (file said {} stream said {})", hdr.archive_size,
    ...
    if (hdr.rect_offset) {
        ...
        for (uint32_t i = 0; i < rect_hdr.image_count; i++) {
            ...
            if (entry.x1 >= entry.x2 || entry.y1 >= entry.y2) {
                log_warning("rect entry has invalid dimensions ({},{},{},{})", entry.x1, entry.x2,
                    entry.y1, entry.y2);
                return std::nullopt;
            }
    ...
    for (auto& [rect_name, rects] : updates) {
        ...
        for (auto& rect : rects) {
            if (rect->x2() > width || rect->y2() > height) {
                log_warning("Can't update rect in {}: out of bounds (canvas is {}x{}, rect is "
                            "x1,x2,y1,y2 {},{},{},{})",
                    rect_name, width, height, rect->x, rect->x2(), rect->y, rect->y2());
                continue;
            }
...
bool Texbin::save(const char* dest) {
    ...
    uint32_t data_offset =
        hdr.data_entry_offset + (uint32_t)(images.size() * sizeof(TexbinDataEntry));
    ...
    if (rects.size()) {
        ...
        for (auto& [name, rect] : rects) {
            ...
            if (parent == images.end()) {
                log_warning(
                    "Rect entry \"{}\" has an invalid parent name \"{}\"", name, rect.parent_name);
                return false;
            }
...
std::optional<std::tuple<std::vector<uint8_t>, uint16_t, uint16_t>>
ImageEntryParsed::tex_to_argb8888() {
...
std::vector<uint8_t> texbin_lz77_decompress(
    const std::vector<uint8_t>& comp_with_hdr, size_t max_len, bool debug) {
    ...
    if (debug) {
        VLOG(
            "%s: Comp sz %u decomp sz %u (clamp: %d)", __FUNCTION__, comp_len, decomp_len, max_len);
    }
